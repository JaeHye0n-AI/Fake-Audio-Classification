...
# def train(model, train_loader, device, optimizer, criterion, study_path):
    for epoch in range(config.num_iters):
        ...
        for inputs, label in tqdm(train_loader):
            ...
            ### aug ###
            aug_data_final = aug_1(inputs)

            aug_data = torch.unsqueeze(aug_data_final, 1)

            inputs = aug_data.to(device)
